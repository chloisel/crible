...
NUMERO_SAISON = 57  # numéro ordinal de la saison en cours — codé en dur, à ajuster chaque nouvelle saison
...
def _page_soiree(story, styles, soiree_info, lignes, absents):
    ...
    story.append(Paragraph(
        f"({NUMERO_SAISON} ième saison - {soiree_vie_num} ième soirée)",
...
def _page_stats_saison(story, styles, saison_label, top8, records, anniversaires):
    story.append(Paragraph('LIGUE DE CRIB "DOUBLE-SKUNK"', styles["TitreLigue"]))
    ...
    data = [["Rang", "Nbr", "Date", entete_perdus, entete_gagnes]]
    for l in top8:
    ...
    if records is None:
        ...
        lignes = [
            ("Plus beau skunk pour une soirée", str(records["plus_de_skunks_nbr"]),
